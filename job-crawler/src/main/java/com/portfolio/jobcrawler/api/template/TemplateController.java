...
@Tag(name = "템플릿", description = "자소서/포트폴리오 템플릿 관리")
@RestController
...
public class TemplateController {
    ...
    /** 시스템 프리셋 조회 (모든 유저) */
    @Operation(summary = "시스템 프리셋 템플릿 목록 조회")
    @GetMapping("/presets")
    ...
    /** AI로 프리셋 갱신 (관리자 전용) */
    @Operation(summary = "AI로 프리셋 템플릿 갱신 (관리자 전용)")
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<Template>> get(Authentication auth, @Parameter(description = "템플릿 ID") @PathVariable Long id) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Template>> update(Authentication auth, @Parameter(description = "템플릿 ID") @PathVariable Long id,
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse<Void>> delete(Authentication auth, @Parameter(description = "템플릿 ID") @PathVariable Long id) {
        templateService.deleteTemplate((Long) auth.getPrincipal(), id);
        return ResponseEntity.ok(ApiResponse.ok());
    }

    @Operation(summary = "기본 템플릿으로 설정")
    @PatchMapping("/{id}/default")
    public ResponseEntity<ApiResponse<Void>> setDefault(Authentication auth, @Parameter(description = "템플릿 ID") @PathVariable Long id) {
