    ...
    private String buildPrompt(AiGenerationRequest request) {
        return switch (request.getType()) {
            case COVER_LETTER -> String.format("""
                    다음 정보를 바탕으로 맞춤형 자기소개서를 작성해주세요.
                    반드시 순수 텍스트로만 작성하세요. 마크다운(*, #, ` 등), HTML 태그(<div>, <p>, <h1> 등) 절대 금지.
                    마지막에 "원하시면", "다음 중 하나로", "이어서", "버전" 같은 후속 제안 문구 절대 넣지 마세요. 자소서 내용만 작성하고 끝내세요.
            ...
            case PORTFOLIO -> String.format("""
                    다음 프로젝트 경험을 바탕으로 포트폴리오 텍스트를 작성해주세요.
                    반드시 순수 텍스트로만 작성하세요. 마크다운(*, #, ` 등), HTML 태그(<div>, <p>, <h1>, <br/> 등) 절대 금지.
                    줄바꿈은 빈 줄로만 구분하세요.
                    마지막에 "원하시면", "활용 팁", "다음 형태", "버전" 같은 후속 제안 문구 절대 넣지 마세요. 포트폴리오 내용만 작성하고 끝내세요.
            ...
            case PROJECT_SUMMARY -> String.format("""
                    ...
                    다음 형식으로 정리:
                    1. 프로젝트 개요 (2~3문장)
                    2. 기술 스택 (사용된 언어/프레임워크/도구)
                    3. 아키텍처 패턴 (MVC, DDD, 마이크로서비스 등)
                    4. 주요 기능 (항목별)
                    5. 특이사항 및 강점
                    """, request.getMatchedProjects());
        };
